...
class ReviewService:
    ...
    @staticmethod
    async def add_review(db: AsyncSession, song_id: int, review: ReviewCreate, current_user_id: int):
        if not current_user_id:
            raise HTTPException(status_code=401, detail="로그인이 필요한 서비스입니다.")
        try:
            new_review = await ReviewCrud.new_review(db, song_id, review, current_user_id)
            await db.commit()
            await db.refresh(new_review)  # 추가
            return new_review
        except Exception:
            await db.rollback()
    ...
    async def update_review(db: AsyncSession, review_id: int, rating: int, comment: str, current_user_id: int):
        ...
        try:
            ...
            updated = await ReviewCrud.update_by_id(db, review_id, review_data,current_user_id)
            await db.commit()
    ...
    async def del_review(db: AsyncSession, review_id: int, current_user_id: int):
        ...
        # 3. 삭제 진행
        await ReviewCrud.delete_by_id(db, review_id,current_user_id)
        await db.commit()
