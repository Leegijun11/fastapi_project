...
from schemas.playlist import PlaylistCreate, PlaylistResponse
...
class PlaylistService:
    ...
    async def get_playlist_with_songs(db, playlist_id:int):
        ...
        if not playlist:
            raise HTTPException(status_code=404, detail="플레이스트가 없다")
        song_ids=await PlaylistsongService.get_song_ids(db, playlist_id)
