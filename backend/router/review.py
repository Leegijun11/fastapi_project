...
async def add_review(response: Response, song_id: int, review: ReviewCreate, db: AsyncSession = Depends(get_db),
                     current_user= Depends(get_current_user)):
    new_review = await ReviewService.add_review(db, song_id, review, current_user.user_id)
    return new_review
