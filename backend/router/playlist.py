...
@router.post("/")
async def new_theme(playlist: PlaylistCreate, response: Response, db: AsyncSession = Depends(get_db),
                    current_user= Depends(get_current_user)):
    db_playlist = await PlaylistService.new_theme(db, current_user.user_id, playlist)
